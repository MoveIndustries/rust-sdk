...
impl<'a> ModuleGenerator<'a> {
    ...
    pub fn generate(&self) -> MovementResult<String> {
        let mut output = String::new();

        self.write_all(&mut output)
            .map_err(|e| MovementError::Internal(format!("Code generation failed: {e}")))?;
    ...
    fn write_events(&self, output: &mut String) -> std::fmt::Result {
        ...
        writeln!(
            output,
            "pub fn parse_event(event_type: &str, data: serde_json::Value) -> MovementResult<ModuleEvent> {{"
        )?;
        ...
        for struct_def in &event_structs {
            ...
            writeln!(
                output,
                "                .map_err(|e| MovementError::Internal(format!(\"Failed to parse {}: {{}}\", e)))?;",
                struct_def.name
            )?;
    ...
    fn write_imports(&self, output: &mut String) -> std::fmt::Result {
        writeln!(output, "use movement_sdk::{{")?;
        writeln!(output, "    account::Account,")?;
        writeln!(output, "    error::{{MovementError, MovementResult}},")?;
        writeln!(
            output,
            "    transaction::{{EntryFunction, TransactionPayload}},"
        )?;
        writeln!(output, "    types::{{AccountAddress, TypeTag}},")?;
        writeln!(output, "    Movement,")?;
        writeln!(output, "}};")?;
    ...
    fn write_entry_function(&self, output: &mut String, func: &MoveFunction) -> std::fmt::Result {
        ...
        writeln!(output, ") -> MovementResult<TransactionPayload> {{")?;
    ...
    fn write_view_function(&self, output: &mut String, func: &MoveFunction) -> std::fmt::Result {
        ...
        write!(output, "pub {async_kw}fn {rust_name}(movement: &Movement")?;
        ...
        writeln!(output, ") -> MovementResult<Vec<serde_json::Value>> {{")?;
        ...
        writeln!(
            output,
            "    movement.view(&function_id, type_args, args){await_kw}"
        )?;
        writeln!(output, "}}")?;
        writeln!(output)
    }
    ...
    fn view_arg_json_expr(&self, move_type: &str, var_name: &str) -> String {
        match move_type {
            ...
            _ if move_type.starts_with("vector<u8>") => {
                format!("serde_json::json!(::movement_sdk::const_hex::encode({var_name}))")
            }
